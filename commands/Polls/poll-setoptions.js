        ...
        Promise.all(promises)
          .then(() => {
            return message.channel.send(
              `${message.author}, your poll's options are set to ${emojis}`
            );
          })
          .catch(console.error);
      } else {
    ...
    const storeOptions = async (options, emojis) => {
      try {
        ...
        if ((await findDuplicates(options).length) > 0) {
          return message.channel.send(
            `Sorry ${message.author}, but you can't set two options to be the same thing.`
          );
        } else {
        ...
        if (isUnicodeEmoji) {
          ...
          if (emoji != undefined) {
            ...
            message.channel.send(
              `My apologies ${message.author}, but there doesn't seem to be an emoji called \`${element}\`.`
            );
          }
        }
      });
