...
module.exports = {
    ...
    async execute(message, args, currency, Users, CurrencyShop) {
        ...
        if(item.cost > currency.getBalance(message.author.id)) {
            return message.channel.send(`Sorry ${message.author}, but you only have ${currencyPrefix}${currency.getBalance(message.author.id)} while ${item.name} costs ${currencyPrefix}${item.cost}!`);
        }
